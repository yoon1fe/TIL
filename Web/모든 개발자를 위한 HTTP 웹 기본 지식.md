...
- 헤더에 `cache-control: max-age=60` -> 60초 동안 캐시가 유효하다
- 이렇게 오면 브라우저 캐시에 응답 결과를 저장한다.

이렇게 되면

- 캐시 덕분에 캐시 가능 시간동안 네트워크를 타지 않아도 된다
- 고로 비싼 네트워크 사용량을 줄일 수 있다
- 브라우저 로딩 속도가 매우 빠르다
- 빠른 사용자 경험
...
근데 만약 캐시에 있는 애랑 새로 받은 애랑 같다면 너무 비효율적이잖아
...
- 캐시 유효 시간이 초과해서 서버에 다시 요청하면 다음 두 가지 상황이 발생할 수 있다
  1. 서버에서 기존 데이터를 변경함
  2. 서버에서 기존 데이터를 변경하지 않음



2번 경우를 보면, 저장해 두었던 캐시를 재사용할 수 있다.

단, 클라이언트(캐시)의 데이터와 서버의 데이터가 같다는 사실을 확인할 수 있는 방법이 필요하다!
...
- 캐시 시간이 초과되어서 다시 서버에 요청할 때, `Last-Modified` 가 있으면 `if-modified-since` 를 헤더에 추가해서 서버에 요청한다. 말그대로 이 뒤로 바뀐 게 있나!?

- 서버에서 요청을 받았을 때, 데이터 최종 수정일이 같다면?? 

  -> `304 Not Modified` 응답을 보낸다!! 여긴 HTTP Body가 없다. body가 없으므로 네트워크 부하가 확 줄겠지

- 클라이언트가 이 응답을 받으면 캐시 시간을 갱신해주면 된다~!
...
- 데이터를 수정해서 날짜가 다르지만, 데이터 결과가 똑같은 경우(a->b->a)를 캐치 못한다
...
- Cache-Control - 캐시 지시어(directives)
  ...
  젤 중요하다. Pragma, Expires 얘들은 하위 호환때문에 쓰고, Cache-Control 하나로 다 제어 가능  

  - max-age - 캐시 유효 시간, 초 단위
  - no-cache - 데이터는 캐시해도 되지만, 항상 원(origin) 서버에 검증하고 사용
  - no-store - 데이터에 민감한 정보가 있으므로 저장하면 안됨 (메모리에서 사용하고 최대한 빨리 삭제하세요)
...
원(origin) 서버 직접 접근하는건 겁내게 오래 걸린다.

프록시 캐시 서버라는 걸 도입한다..(CDN.. 한국 어딘가에 있다)

프록시 캐시 서버를 public 캐시, 웹 브라우저에 있는 캐시를 private 캐시라고 한다.
...
- `Cache-Control: private`
  ...
  응답이 해당 사용자만을 위한 것이다. private 캐시에 저장해야 한다(default)
...
#### Cache-Control 의 확실한 캐시 무효화 응답

이건 절대 캐시 되면 안돼! 하는 것들

은행 계좌 잔고 같은게 있을 수 있겠다.

- no-cache, no-store, must-revalidate 다 써야 한다.

  - must-revalidate - 캐시 만료 후 최초 조회 시 **원 서버**에 검증해야 함
...
만약에 no-cache 와서 프록시 캐시쪽에서 원 서버로 통신하는 순간 네트워크가 단절된다면?!

-> 오류보다는 오래된 데이터라도 보여주자! 라는 설정할 수도 있다.



만약 must-revalidate일때 같은 상황이라면?!

-> 504 Gateway Timeout! (돈과 같이 주요한 데이터인 경우 필요하겠다)
